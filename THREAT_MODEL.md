...
| Thing | Why | Limit |
|---|---|---|
| Your config file | Only you (and programs running as you) can write it. It is the single place a server can be added. | `0600` key file; config lives in your user config directory. |
...
- `webmcp discover` reads other tools' MCP configs read-only and prints
  ...
  gets its keys; the command says which variables were copied.
...
killed when the session ends, when the server is disabled or removed, or when
the connection drops. A lock file prevents two daemons serving one device.
